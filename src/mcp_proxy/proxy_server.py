...
async def create_proxy_server(remote_app: ClientSession, callback: None) -> server.Server[object]:  # noqa: C901, PLR0915
    ...
    if capabilities.tools:
        ...
        async def _call_tool(req: types.CallToolRequest) -> types.ServerResult:
            try:
                await callback("call_tool", {"action": "begin", "tool": req.params.name, "arguments": req.params.arguments or {} })
                ...
                for item in result.content:
                    if isinstance(item, mcp.types.TextContent):
                        ...
                        result_content.append({"type": item.type, "data": item.data, "mimeType": item.mimeType})
                    elif isinstance(item, mcp.types.EmbeddedResource):
                        result_content.append({"type": item.type})
                await callback("call_tool", {"action": "end", "tool": req.params.name, "arguments": req.params.arguments or {} , "result": result_content})
                return types.ServerResult(result)
